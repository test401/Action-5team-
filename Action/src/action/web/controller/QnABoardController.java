...
import action.business.service.board.QnABoardService;
import action.business.service.board.QnABoardServiceImpl;
import action.util.PageHandler;

/**
 * Servlet implementation class QnABoardController
 */
public class QnABoardController extends HttpServlet {
	...
	private void selectBoardList(HttpServletRequest request, HttpServletResponse response)
		...
		QnABoardService boardService = new QnABoardServiceImpl();		
		...
		QnABoard[] boardList = boardService.getBoardList(searchInfo);
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/views/qnaBoard/list.jsp");
	...
	private void readBoard(HttpServletRequest request, HttpServletResponse response) 
		...
		QnABoardService boardService = new QnABoardServiceImpl();
		QnABoard board = boardService.findBoard(num);
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/views/qnaBoard/read.jsp");
		dispatcher.forward(request, response);
	...
	private void writeBoardForm(HttpServletRequest request, HttpServletResponse response) 
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/views/qnaBoard/writeForm.jsp");
		dispatcher.forward(request, response);
	}
	...
	private void updateBoardForm(HttpServletRequest request, HttpServletResponse response) 
		...
		QnABoardService boardService = new QnABoardServiceImpl();
		QnABoard board = boardService.findBoard(boardNum);
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/views/qnaBoard/updateForm.jsp");
		dispatcher.forward(request, response);
	}
	...
	private void updateBoard(HttpServletRequest request, HttpServletResponse response) 
		...
		// 2. 구해 온 요청 파라미터 값와 ip 값을 지닌 Board 객체를 생성한다.
		QnABoard board = new QnABoard(boardNum, title, contents);
